...
export function DailyPage() {
  ...
  const handleCreateTask = async (data: {
    ...
    if (editingTodo) {
      ...
      await addTodo({ ...data, category: 'daily' })
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-gray-900 dark:text-white tracking-tight flex items-center gap-2">
            ...
            <span>Daily Tasks Monitoring</span>
          </h1>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            Focus on today's high-impact actions and daily habits.
          </p>
        </div>
      ...
      <div className="glass-card p-6 rounded-3xl grid grid-cols-1 md:grid-cols-3 gap-6 items-center border border-gray-200/80 dark:border-gray-800">
        <div className="md:col-span-2 space-y-3">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-xl bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-xs font-bold">
            <Clock size={14} />
            <span>24h Daily Cycle</span>
          </div>
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">
            Daily Completion Strategy
          </h2>
          <p className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed">
            Keep momentum going by breaking your day into achievable steps. Reorder tasks by priority using drag and drop handles.
          </p>
      ...
      {loading ? (
        ...
        <TodoList
          ...
          emptyMessage="No daily tasks found. Create a task to start your daily focus!"
        />
      )}
